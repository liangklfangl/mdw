...
export default class DemoPage extends React.Component{
 ...
 handleExpandToggle = ()=>{
  ...
  render(){
   ...
   const demos = R.values(this.props.demos);
   const leftDemos = [];
   const rightDemos = [];
   //分为左边的Demo和右边两处
   ...
   demos.forEach((DemoData,index)=>{
     ...
     //获取到demo页面的预览数据，jsonmlReactLoader={demos[key]["jsonmlReactLoader"]}
     if(index % 2 == 0){
      //父组件要通知子组件重新渲染才行
     	  leftDemos.push(
              <Demo key={index} {...DemoData}  codeExpand={this.state.expandAll} utils={utils} />
     	  	);
     }else{
     	  rightDemos.push(
             <Demo key={index} {...DemoData} codeExpand={this.state.expandAll} utils={utils} />
