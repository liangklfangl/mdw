...
/**
 * [findMDFile Find markdown file in folder]
...
function findMDFile(source) {
  return R.pipe(R.filter(R.either(isDirectory, isMDFile)), R.chain(function (filename) {
    if (isDirectory(filename)) {
      ...
      //Compatiable in win platform, we here do not join with '/' 
...
function filesToTreeStructure(files) {
  return files.reduce(function (filesTree, filename) {
    var lenProp = filename.replace(/\.md$/i, '').split(path.sep);
    var propLens = R.lensPath(lenProp);
    //If replace one value of object , we can use lensProp instead!
...
function stringifyObject(nodePath, obj, lazyLoad, isSSR, depth) {
  ...
  var kvStrings = R.pipe(R.toPairs, R.map(function (kv) {
    return (//这每一个rv对象都是这样的['a','1']第一个元素是key，第二个为value
...
function stringify(nodePath, nodeValue, lazyLoad, isSSR, depth) {
  ...
  return R.cond([[function (n) {
    ...
    var filePath = path.join(process.cwd(), filename).split(path.sep).join("/");
    if (shouldBeLazy) {
...
exports.stringify = function (filesTree, lazyLoad, isSSR) {
  return stringify('/', filesTree, lazyLoad, isSSR, 0);
};
...
/**
 * [We process jsonml data with all plugin configured]
...
var _temp = function () {
  ...
  __REACT_HOT_LOADER__.register(ensureToBeArray, 'ensureToBeArray', 'src/utils/markdownData.js');

  __REACT_HOT_LOADER__.register(isDirectory, 'isDirectory', 'src/utils/markdownData.js');

  __REACT_HOT_LOADER__.register(isMDFile, 'isMDFile', 'src/utils/markdownData.js');

  __REACT_HOT_LOADER__.register(findMDFile, 'findMDFile', 'src/utils/markdownData.js');

  __REACT_HOT_LOADER__.register(rxSep, 'rxSep', 'src/utils/markdownData.js');

  __REACT_HOT_LOADER__.register(filesToTreeStructure, 'filesToTreeStructure', 'src/utils/markdownData.js');

  __REACT_HOT_LOADER__.register(stringifyObject, 'stringifyObject', 'src/utils/markdownData.js');

  __REACT_HOT_LOADER__.register(lazyLoadWrapper, 'lazyLoadWrapper', 'src/utils/markdownData.js');

  __REACT_HOT_LOADER__.register(shouldLazyLoad, 'shouldLazyLoad', 'src/utils/markdownData.js');

  __REACT_HOT_LOADER__.register(stringify, 'stringify', 'src/utils/markdownData.js');
}();
